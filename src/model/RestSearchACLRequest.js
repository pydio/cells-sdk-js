/**
 * Pydio Cells Rest API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 4.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

import ApiClient from '../ApiClient';
import IdmACLSingleQuery from './IdmACLSingleQuery';
import ServiceOperationType from './ServiceOperationType';

/**
 * The RestSearchACLRequest model module.
 * @module model/RestSearchACLRequest
 * @version 4.0
 */
class RestSearchACLRequest {
    /**
     * Constructs a new <code>RestSearchACLRequest</code>.
     * @alias module:model/RestSearchACLRequest
     */
    constructor() { 
        
        RestSearchACLRequest.initialize(this);
    }

    /**
     * Initializes the fields of this object.
     * This method is used by the constructors of any subclasses, in order to implement multiple inheritance (mix-ins).
     * Only for internal use.
     */
    static initialize(obj) { 
    }

    /**
     * Constructs a <code>RestSearchACLRequest</code> from a plain JavaScript object, optionally creating a new instance.
     * Copies all relevant properties from <code>data</code> to <code>obj</code> if supplied or a new instance if not.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @param {module:model/RestSearchACLRequest} obj Optional instance to populate.
     * @return {module:model/RestSearchACLRequest} The populated <code>RestSearchACLRequest</code> instance.
     */
    static constructFromObject(data, obj) {
        if (data) {
            obj = obj || new RestSearchACLRequest();

            if (data.hasOwnProperty('CountOnly')) {
                obj['CountOnly'] = ApiClient.convertToType(data['CountOnly'], 'Boolean');
            }
            if (data.hasOwnProperty('GroupBy')) {
                obj['GroupBy'] = ApiClient.convertToType(data['GroupBy'], 'Number');
            }
            if (data.hasOwnProperty('Limit')) {
                obj['Limit'] = ApiClient.convertToType(data['Limit'], 'String');
            }
            if (data.hasOwnProperty('Offset')) {
                obj['Offset'] = ApiClient.convertToType(data['Offset'], 'String');
            }
            if (data.hasOwnProperty('Operation')) {
                obj['Operation'] = ServiceOperationType.constructFromObject(data['Operation']);
            }
            if (data.hasOwnProperty('Queries')) {
                obj['Queries'] = ApiClient.convertToType(data['Queries'], [IdmACLSingleQuery]);
            }
        }
        return obj;
    }

    /**
     * Validates the JSON data with respect to <code>RestSearchACLRequest</code>.
     * @param {Object} data The plain JavaScript object bearing properties of interest.
     * @return {boolean} to indicate whether the JSON data is valid with respect to <code>RestSearchACLRequest</code>.
     */
    static validateJSON(data) {
        // ensure the json data is a string
        if (data['Limit'] && !(typeof data['Limit'] === 'string' || data['Limit'] instanceof String)) {
            throw new Error("Expected the field `Limit` to be a primitive type in the JSON string but got " + data['Limit']);
        }
        // ensure the json data is a string
        if (data['Offset'] && !(typeof data['Offset'] === 'string' || data['Offset'] instanceof String)) {
            throw new Error("Expected the field `Offset` to be a primitive type in the JSON string but got " + data['Offset']);
        }
        if (data['Queries']) { // data not null
            // ensure the json data is an array
            if (!Array.isArray(data['Queries'])) {
                throw new Error("Expected the field `Queries` to be an array in the JSON data but got " + data['Queries']);
            }
            // validate the optional field `Queries` (array)
            for (const item of data['Queries']) {
                IdmACLSingleQuery.validateJSON(item);
            };
        }

        return true;
    }


}



/**
 * @member {Boolean} CountOnly
 */
RestSearchACLRequest.prototype['CountOnly'] = undefined;

/**
 * @member {Number} GroupBy
 */
RestSearchACLRequest.prototype['GroupBy'] = undefined;

/**
 * @member {String} Limit
 */
RestSearchACLRequest.prototype['Limit'] = undefined;

/**
 * @member {String} Offset
 */
RestSearchACLRequest.prototype['Offset'] = undefined;

/**
 * @member {module:model/ServiceOperationType} Operation
 */
RestSearchACLRequest.prototype['Operation'] = undefined;

/**
 * @member {Array.<module:model/IdmACLSingleQuery>} Queries
 */
RestSearchACLRequest.prototype['Queries'] = undefined;






export default RestSearchACLRequest;

